...
def compute_hurst_dfa(data, use_cpu=False):
    """
    Compute Hurst exponent using Detrended Fluctuation Analysis (DFA) for each channel.
    Args:
        data: Tensor of shape [N, L, C] (samples, sequence length, channels)
        use_cpu: If True, compute on CPU
    Returns:
        hurst: Tensor of shape [C], Hurst exponent per channel
    """
    if use_cpu:
        data = data.cpu()
    else:
    ...
    for c in range(C):
        ...
        for scale in scales:
            ...
            # Compute cumulative sum and detrend
        ...
        if valid and len(fluctuations) > 1:
            # Fit log(fluctuation) vs log(scale)
...
def torch_polyfit(x, y, deg):
    """
    Polynomial fit for PyTorch tensors.
    Args:
        x: Tensor of shape [N], independent variable (e.g., time points)
        y: Tensor of shape [..., N], dependent variable (e.g., data to fit)
        deg: Degree of polynomial (e.g., 1 for linear)
    Returns:
        coeffs: Tensor of shape [..., deg+1], polynomial coefficients
    """
    device = y.device
    scale = x.shape[0]
    N, n_segments, _ = y.shape  # y: [N, n_segments, scale]

    # Create Vandermonde matrix for polynomial of degree `deg`
    powers = torch.arange(deg + 1, device=device).flip(0)  # [deg, deg-1, ..., 0]
    X = x.unsqueeze(-1).pow(powers)  # [scale, deg+1]

    # Prepare y for lstsq: [N, n_segments, scale, 1]
    y = y.unsqueeze(-1)  # [N, n_segments, scale, 1]

    # Broadcast X to [N, n_segments, scale, deg+1]
    X = X.view(1, 1, scale, deg + 1).expand(
        N, n_segments, -1, -1
    )  # [N, n_segments, scale, deg+1]

    # Solve least squares: X @ coeffs = y
    # X: [N, n_segments, scale, deg+1], y: [N, n_segments, scale, 1]
    # coeffs: [N, n_segments, deg+1, 1]
    coeffs = torch.linalg.lstsq(X, y).solution  # [N, n_segments, deg+1, 1]

    # Reshape coeffs to [N, n_segments, deg+1]

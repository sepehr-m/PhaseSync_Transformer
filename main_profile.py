...
from model.PSTransformer import PSTransformer
from data_loader import get_loader
from utils.utils import kl_loss, adjust_learning_rate, compute_hurst_dfa, EarlyStopping
...
class AnomalyDetection:
    def __init__(self, config_path, dataset):
        ...
        self.sigma = self.config["model"]["sigma"]
    ...
    def train(self):
        ...
        # Profiler setup for training
        with torch.profiler.profile(
            activities=[torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.CUDA],  # Adjust for your device (CPU/CUDA/MPS)
            schedule=torch.profiler.schedule(wait=1, warmup=1, active=3, repeat=1),  # Profile a few epochs
            on_trace_ready=torch.profiler.tensorboard_trace_handler(f'logs/profile_train_{self.dataset}'),  # Save to TensorBoard
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        ) as prof:
            ...
            for epoch in range(self.n_epochs):
                ...
                prof.step()  # Step the profiler after each epoch

        # Export additional profiler outputs
        prof.export_chrome_trace(f"profile_trace_train_{self.dataset}.json")
        logger.info(f"Training profiler trace saved for {self.dataset}")
    ...
    def test(self):
        ...
        # Profiler setup for testing
        with torch.profiler.profile(
            activities=[torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.CUDA],  # Adjust for your device
            schedule=torch.profiler.schedule(wait=1, warmup=1, active=3, repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(f'logs/profile_test_{self.dataset}'),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        ) as prof:
            ...
            prof.step()  # Step if multiple iterations in test

        # Export additional profiler outputs for test
        prof.export_chrome_trace(f"profile_trace_test_{self.dataset}.json")
        logger.info(f"Testing profiler trace saved for {self.dataset}")
